...
int model::load_model(string model_name) {
    ...
    for (i = 0; i < M; i++) {
        ...
        if (!pointer) {
            printf(
                    "Invalid word-topic assignment file, check the number of docs!\n");
            return 1;
        }
...
int model::init_inf() {
    ...
    if (withrawstrs) {
        if (pnewdata->read_newdata_withrawstrs(dir + dfile,
                dir + wordmapfile)) {
...
int model::inf_sampling(int m, int n) {
    ...
    // do multinomial sampling via cumulative method
    for (int k = 0; k < K; k++) {
        p[k] = (nw[w][k] + newnw[_w][k] + beta)
                / (nwsum[k] + newnwsum[k] + Vbeta) * (newnd[m][k] + alpha)
                / (newndsum[m] + Kalpha);
    }
    // cumulate multinomial parameters
    for (int k = 1; k < K; k++) {
